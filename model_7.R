...
lm_model <- train(Rating~.,newtrain,'lm',trControl = tr)
#plot(xgbLinear_model)

prediction <- predict(lm_model,newtrain)
RMSE(prediction,newtrain$Rating)

#testing
prediction <- predict(lm_model,newtest)

submission <- data.frame('ID'=dtest$ID,'Rating'=prediction)
write.csv(submission, 'Submission_lm_v5.csv',row.names = F)
